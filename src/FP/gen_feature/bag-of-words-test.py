...
def link_fextract(folder,keyword,html_file):
    ...
    for line in f:
        if line.split('=>')[1].strip() == 'http://':
            continue
        if line.split('=>')[1].startswith('es/') or line.split('=>')[1].startswith('c/') or line.split('=>')[1].startswith('%20/') or line.split('=>')[1].startswith('images/') or (line.split('=>')[1].startswith('http') and (keyword in line.split('=>')[1])) or line.split('=>')[1].startswith('/') or line.split('=>')[1].startswith('javascript') or line.split('=>')[1].startswith('#') or 'mailto:' in line:
            n_glinks += 1
...
if __name__ == "__main__":
    '''
        This is the script to generate design-level features for testing file.
    ...
    bag_file='bag-new-300-test.csv'#'bag-wf-popular.csv'
    """    """
