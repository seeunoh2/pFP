...
exp = 'close'
n_epoch1 = 60
folder='buflo_oh_0.02_10' # should be under ~/feature
r_train = 60.0
r_test = 40.0
nClass = 100
mon_instance = 300.0
...
for iter in range(n_iter):
    ...
    (X, Y, testX, testY) = data.split_close_defense('feature/' + folder, r_train, r_test, 100.0, mon_instance, dim)
    ...
    model.fit(X, Y, n_epoch=n_epoch1, validation_set=0.1, show_metric=True, run_id="fullnet_buflo"+str(iter))
    # Save model
    model.save(os.path.join(model_path, 'Fullnet_buflo_' + str(iter) + '.tflearn'))
    ...
    # top k accuracy
    k_list = [1, 2, 3]
    exp_defense=folder#'wtfpad-pfp'
    for topK in k_list:
        for conf in confs:
            file = os.path.join(HOME, 'results', 'Fullnet_Buflo' + '_top_' + str(topK) + '_' + gb + str(dim) + '_' + str(
