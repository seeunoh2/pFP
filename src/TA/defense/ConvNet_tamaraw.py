...
for iter in range(n_iter):
    ...
    (X, Y, testX, testY) = data.split_close_tamaraw('feature/' + folder, 60.0, 40.0, nClass, mon_instance, dim)
    if exp == 'open':
        nClass = nClass + 1
    if b_label:
    ...
    model.fit({'input': X}, {'target': Y}, n_epoch=n_epoch1, validation_set=0.1, show_metric=True,
              snapshot_step=100, run_id='convnet_tamaraw_'+str(iter))
    # Save model
    model.save(os.path.join(model_path, 'Conv_tamaraw_' + str(iter) + '.tflearn'))
    ...
    for topK in k_list:
        file = os.path.join(HOME, 'results', 'Conv_Tamaraw_' + '_top_' + str(topK) + '_' + gb + str(
dim) + '_' + str(nClass) + '_' + str(dim) + '.txt')
