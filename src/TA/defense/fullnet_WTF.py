...
for iter in range(n_iter):
    ...
    model.fit(X, Y, n_epoch=n_epoch1, validation_set=0.1,
                  show_metric=True, run_id='Fullnet_WTF_' + str(iter))
    # Save model
    model.save(os.path.join(model_path, 'Fullnet_WTF_' + str(iter) + '.tflearn'))
    ...
    for topK in k_list:
        for conf in confs:
            file = os.path.join(HOME, 'results',
                                'Fullnet_WTF' + '_top_' + str(topK) + '_' + gb + str(dim) + '_' + str(
