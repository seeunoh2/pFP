...
folder_mon = 'feature/ssl_mon_tcp'  # should be under ~/feature
folder_unmon = 'feature/ssl_unmon_tcp'  # 'Downloads/Unmonitored'
r_train = 60.0
r_test = 40.0
b_label = False
if b_label:
...
for fold in range(n_fold):
    ...
    (X, Y, testX, testY, allX, allY) = data.split_ssl(folder_mon, folder_unmon, r_train, r_test, nClass,
                                                      mon_instance, unClass, unmon_instance, dim)
    if exp == 'open':
        nClass = nClass + 1
    if b_label:
    ...
    model.fit({'input': X}, {'target': Y}, validation_set=0.1, n_epoch=n_epoch1, snapshot_step=100,
              run_id='Conv_ssl_tcp_' + str(fold) + '.tflearn')
    # Save model
    model.save(os.path.join(model_path, 'Conv_ssl_tcp_' + str(fold) + '.tflearn'))
    ...
    for topK in k_list:
        if not b_label:
            ...
            file = os.path.join(HOME, 'results',
                                'Conv_ssl_tcp_' + exp + '_' + gb + str(dim) + '_' + str(
                                    nClass) + '_' + str(unClass) + '_' + str(n_epoch1) + '.txt')
