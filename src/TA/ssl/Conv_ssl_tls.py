...
gb = 'wf'
mon_instance = 90.0
folder_mon = 'feature/ssl_mon'  # should be under ~/feature
folder_unmon = 'feature/ssl_unmon'
r_train = 60.0
r_test = 40.0
...
for fold in range(n_fold):
    ...
    model.fit({'input': X}, {'target': Y}, validation_set=0.1, n_epoch=n_epoch1, snapshot_step=100,
              run_id='Conv_ssl_tls_' + str(fold) + '.tflearn')
    # Save model
    model.save(os.path.join(model_path, 'Conv_ssl_tls_' + str(fold) + '.tflearn'))
    ...
    for topK in k_list:
        if not b_label:
            ...
            file = os.path.join(HOME, 'results',
                                'Conv_ssl_tls_' + exp + '_' + gb + str(dim) + '_' + str(
